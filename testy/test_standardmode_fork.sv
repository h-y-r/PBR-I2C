...
always @(negedge testbench.SDA) begin
    if (testbench.SCL == 1)
        $display("[%0t] START-like SDA fall, phase=%0d", $realtime, `DRIVER.phase);
end

always @(negedge testbench.SCL) begin
    $display("[%0t] SCL fall, phase=%0d", $realtime, `DRIVER.phase);
end
...
task automatic check_repeated_start_setup_time();
    ...
    @(posedge testbench.SCL iff (`DRIVER.phase == M_START));
...
task automatic check_stop_setup_time();
    ...
    @(posedge testbench.SCL iff (`DRIVER.phase == M_STOP));

    t_scl_rise_local = $realtime;

    @(posedge testbench.SDA iff (testbench.SCL == 1 && `DRIVER.phase == M_STOP));
...
task automatic check_data_setup_time();
    ...
    @(posedge testbench.SCL iff (`DRIVER.phase == M_DATA_RX || `DRIVER.phase == M_DATA_TX));
...
task automatic check_data_hold_time();
    ...
    @(testbench.SDA iff (`DRIVER.phase == M_DATA_RX || `DRIVER.phase == M_DATA_TX));
